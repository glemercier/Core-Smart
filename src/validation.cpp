...
bool AcceptToMemoryPoolWorker(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fOverrideMempoolLimit, bool fRejectAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    AssertLockHeld(cs_main);
    if (pfMissingInputs)
...
bool GetAddressUnspent(uint160 addressHash, int type, std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >& unspentOutputs, const CAddressUnspentKey& start, int offset, int limit, bool reverse)
{
    if (!fAddressIndex)
        return error("address index not enabled");
...
bool GetDepositIndex(uint160 addressHash, int type, std::vector<std::pair<CDepositIndexKey, CDepositValue> >& depositIndex, int start, int offset, int limit, bool reverse)
{
    if (!fDepositIndex)
        return error("deposit index not enabled");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false, bool fIsVerifyDB = false)
{
    ...
    */
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        //if (fProcessRewards)
        //    prewards->GetEligiblesEntries(smartRewardsResult, nCurrentRewardsRound);
        ...
        BOOST_FOREACH (const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH (const CTxIn& txin, tx.vin) {
                ...
                if (instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(SMART): transaction %s conflicts with transaction lock %s", tx.GetHash().ToString(), hashLocked.ToString()),
...
{
    if (!tx.IsVoteKeyRegistration())
        return VoteKeyParseResult::IsNoRegistrationTx;
